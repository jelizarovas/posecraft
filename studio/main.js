...
let sceneSelection=store.document.emitters?.length?{kind:'emitter',id:(store.document.emitters.find(e=>e.type==='flame')||store.document.emitters[0]).id}:null, sidebarView=store.document.emitters?.length||demo?.id==='campfire-night'?'scene':'character', effectsTime=0;
...
function mount(){renderer=mountSVG($('art'),store.document,frame(),{bones:!sceneSelection&&!selectedProp&&bones,limits,selectedActor:selectedProp||sceneSelection?undefined:selected,selectedJoint:joint,physicsDebug:inspector==='feel',colliders,selectedProp});}
function refresh(){renderer.update(frame());updateTime();if($('rotation')){$('rotation').value=baseValue();$('rotation-number').value=Math.round(baseValue()*(['bend','opacity'].includes(channel())?100:10))/(['bend','opacity'].includes(channel())?100:10);}}
function rebuild(){
 controller?.dispose();if(sceneSelection&&!store.document[{group:'groups',actor:'actors',prop:'props',emitter:'emitters'}[sceneSelection.kind]]?.some(e=>e.id===sceneSelection.id))sceneSelection=null;if(!prop())selectedProp=null;if(!actor())selected=store.document.actors[0]?.id;
...
function selectJoint(actorId,jointId){sceneSelection=null;sidebarView='character';selectedProp=null;if(selected!==actorId){controller.clearPreview(selected);overrides={};}selected=actorId;joint=jointId;if(!pack().clips[clip])clip=Object.keys(pack().clips)[0];previewTime=Math.min(previewTime,pack().clips[clip].duration);inspector='pose';drawHierarchy();drawInspector();drawTimeline();drawDemo();mount();}
...
const sceneTools=createSceneTools({getDocument:()=>store.document,getSelection:()=>sceneSelection,select:selectSceneItem,commit:transact,editEntity:s=>{sceneSelection=null;sidebarView='character';if(s.kind==='actor'){selected=s.id;selectJoint(s.id,pack().joints[0].id);}else selectProp(s.id);drawSceneSidebar();drawTimeline();},play:()=>{playing=true;},pause:()=>{playing=false;},isPlaying:()=>playing&&!reduced(),getTime:()=>effectsTime,setTime:t=>{effectsTime=t;refresh();}});
...
$('scene-workspace').onclick=()=>{sidebarView='scene';drawSceneSidebar();};$('character-workspace').onclick=()=>{sceneSelection=null;sidebarView='character';drawHierarchy();drawInspector();drawTimeline();mount();};
const timelineTools=createTimelineTools({
 ...
 seek:(time,track)=>{const split=track.lastIndexOf('.');joint=track.slice(0,split);poseChannel=spatialChannels[track.slice(split+1)]?track.slice(split+1):'rotation';drawHierarchy();drawInspector();seek(time);mount();},
...
$('duplicate').onclick=()=>addActor(actor().pack,true);$('delete').onclick=()=>{const next=removeSceneEntity(store.document,'actor',selected);transact(['actors','emitters','lighting','ensemble'].filter(k=>next[k]!==undefined||store.document[k]!==undefined).map(k=>next[k]===undefined?{op:'delete',path:[k]}:set([k],next[k])));};
