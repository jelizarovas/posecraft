...
function drawPropInspector(){
 ...
 $('remove-prop').onclick=()=>{const next=removeSceneEntity(store.document,'prop',p.id);transact(['props','contacts'].filter(k=>next[k]!==undefined).map(k=>set([k],next[k])));};
}
