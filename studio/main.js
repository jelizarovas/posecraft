...
$('duplicate').onclick=()=>addActor(actor().pack,true);$('delete').onclick=()=>{const next=removeSceneEntity(store.document,'actor',selected);transact(['actors','emitters','lighting','ensemble','contacts','behaviorGraph','interactions','fluid','objects','objectGames','motionLayers','actorBehaviors','scroll'].filter(k=>next[k]!==undefined||store.document[k]!==undefined).map(k=>next[k]===undefined?{op:'delete',path:[k]}:set([k],next[k])));};
