...
function rebuild(){
 bottleControls?.dispose();pointers?.dispose();controller?.dispose();if(sceneSelection&&sceneSelection.kind!=='behavior'&&!store.document[{group:'groups',actor:'actors',prop:'props',emitter:'emitters'}[sceneSelection.kind]]?.some(e=>e.id===sceneSelection.id))sceneSelection=null;if(!prop())selectedProp=null;if(!actor())selected=store.document.actors[0]?.id;
...
$('duplicate').onclick=()=>addActor(actor().pack,true);$('delete').onclick=()=>{const next=removeSceneEntity(store.document,'actor',selected);transact(['actors','emitters','lighting','ensemble','contacts','behaviorGraph','interactions','fluid'].filter(k=>next[k]!==undefined||store.document[k]!==undefined).map(k=>next[k]===undefined?{op:'delete',path:[k]}:set([k],next[k])));};
