import {createGameTools} from './game-tools.js';
import {createAnimationPreviewTools} from './animation-preview-tools.js';
...
<aside class="sidebar right"><nav class="inspector-tabs"><button data-panel="pose" class="active">${icon('edit')}Pose</button><button data-panel="look">${icon('palette')}Look</button><button data-panel="surface">${icon('polyline')}Surface</button><button data-panel="motion">${icon('animation')}Motion</button><button data-panel="feel">${icon('gesture')}Feel</button><button data-panel="light">${icon('palette')}Light</button></nav><div id="inspector"></div></aside></main>
...
function mount(){bottleControls?.dispose();pointers?.dispose();renderer?.dispose();renderer=(sceneSelection?mountRenderer:mountSVG)($('art'),store.document,frame(),{bones:!sceneSelection&&!selectedProp&&inspector!=='surface'&&bones,limits:!sceneSelection&&limits,selectedActor:selectedProp||sceneSelection?undefined:selected,selectedJoint:joint,physicsDebug:!sceneSelection&&inspector==='feel',colliders:!sceneSelection&&colliders,selectedProp});pointers=mountScenePointers($('art'),store.document,controller,{isEnabled:()=>!!sceneSelection,onInteract:()=>{playing=true;controller.play();}});bottleControls=mountBottleControls($('art'),store.document,controller,{isEnabled:()=>!!sceneSelection,onInteract:()=>{playing=true;controller.play();},onStatus:toast});}
function refresh(){renderer.update(frame());animationGuides.update();meshTools.update(frame(),$('art').querySelector('svg'),inspector==='surface'&&!sceneSelection&&!selectedProp);contactTools.update(frame(),$('art').querySelector('svg'),sceneTools.contactsActive());updateTime();if($('rotation')){$('rotation').value=baseValue();$('rotation-number').value=Math.round(baseValue()*(['bend','opacity'].includes(channel())?100:10))/(['bend','opacity'].includes(channel())?100:10);}}
function rebuild(){
 bottleControls?.dispose();pointers?.dispose();controller?.dispose();if(sceneSelection&&!['behavior','actor-behavior','objects','scroll','motion-layers','game'].includes(sceneSelection.kind)&&!store.document[{group:'groups',actor:'actors',prop:'props',emitter:'emitters'}[sceneSelection.kind]]?.some(e=>e.id===sceneSelection.id))sceneSelection=null;if(!prop())selectedProp=null;if(!actor())selected=store.document.actors[0]?.id;
...
function drawInspector(){
 $('inspector').classList.toggle('mesh-panel',inspector==='surface'&&!sceneSelection&&!selectedProp);
 ...
 if(inspector==='surface'){meshTools.render($('inspector'));return;}
 if(inspector==='pose'){
...
function seek(time){if(!Number.isFinite(time)||!pack())return;previewTime=clamp(time,0,pack().clips[clip].duration);playing=false;overrides={};refresh();$('timeline-content').querySelectorAll('[data-time]').forEach(node=>node.classList.toggle('selected',Math.abs(Number(node.dataset.time)-previewTime)<.011));}
...
const meshTools=createMeshTools({getDocument:()=>store.document,getActor:()=>actor(),getFrame:()=>frame(),commit:transact,notice:toast,pause:()=>{playing=false;controller.pause();}});
...
const behaviorTools=createBehaviorTools({getDocument:()=>store.document,commit:transact,getController:()=>controller,getBottleControls:()=>bottleControls,play:()=>{playing=true;controller.play();},notify:toast});
const objectTools=createObjectTools({getDocument:()=>store.document,commit:transact,getController:()=>controller,notify:toast});
const flowTools=createFlowTools({getDocument:()=>store.document,commit:transact,getController:()=>controller,notify:toast});
const actorBehaviorTools=createActorBehaviorTools({getDocument:()=>store.document,commit:transact,getController:()=>controller,play:()=>{playing=true;controller.play();},notify:toast});
const gameTools=createGameTools({getDocument:()=>store.document,commit:transact});
const sceneTools=createSceneTools({game:gameTools,getFrame:()=>frame(),actorBehaviors:actorBehaviorTools,flows:flowTools,objects:objectTools,behaviors:behaviorTools,contacts:contactTools,getDocument:()=>store.document,getSelection:()=>sceneSelection,select:selectSceneItem,commit:transact,editEntity:s=>{sceneSelection=null;sidebarView='character';if(s.kind==='actor'){selected=s.id;selectJoint(s.id,pack().joints[0].id);}else selectProp(s.id);drawSceneSidebar();drawTimeline();},play:()=>{playing=true;controller.play();},pause:()=>{playing=false;controller.pause();},isPlaying:()=>playing&&!reduced(),getTime:()=>effectsTime,setTime:t=>{effectsTime=t;refresh();},getSceneTime:()=>controller?.frame().time||0,setSceneTime:t=>{effectsTime=t;controller.seek(t);refresh();}});
...
const animationGuides=createAnimationPreviewTools({stage:$('stage'),host:document.querySelector('.viewport'),getContext:()=>({available:!!actor()&&!sceneSelection&&!selectedProp&&tab==='timeline'&&inspector!=='surface',playing:playing&&!reduced(),document:store.document,actor:selected,clip,joint,time:previewTime,overrides,frame}),pause:()=>{playing=false;controller.pause();updateTime();}});
const timelineTools=createTimelineTools({
 getContext:()=>actor()?({packId:actor().pack,clipId:clip,clip:pack().clips[clip],name:actor().name,retimeSummary:`This pack is used by ${store.document.actors.filter(a=>a.pack===actor().pack).length} character(s). Matching contact windows, fades and markers move together.`,canUndo:!!store.past.length,canRedo:!!store.future.length}):null,
 commit:next=>{store.transact([set(['packs',actor().pack,'clips',clip],next)]);overrides={};playing=false;persist();rebuild();},
 seek:(time,track)=>{if(track){const split=track.lastIndexOf('.');joint=track.slice(0,split);poseChannel=editChannels[track.slice(split+1)]?track.slice(split+1):'rotation';drawHierarchy();drawInspector();drawTimeline();}seek(time);mount();},
 retime:duration=>{const result=retimeSceneClip(store.document,{packId:actor().pack,clipId:clip,duration});store.transact(result.commands,result.expectedRevision);previewTime*=result.scale;overrides={};playing=false;persist();rebuild();return result;},
...
$('duplicate').onclick=()=>addActor(actor().pack,true);$('delete').onclick=()=>{const next=removeSceneEntity(store.document,'actor',selected);for(const p of next.props||[])if(store.document.props.find(v=>v.id===p.id)?.attachment&&!p.attachment){const live=evaluatedProps(store.document,frame()).find(v=>v.id===p.id);if(live&&!live.hidden)Object.assign(p,{x:live.x,y:live.y,rotation:wrapAngle(live.rotation)});}transact(['actors','props','emitters','lighting','ensemble','contacts','behaviorGraph','interactions','fluid','objects','objectGames','motionLayers','actorBehaviors','scroll','poseBindings','game'].filter(k=>next[k]!==undefined||store.document[k]!==undefined).map(k=>next[k]===undefined?{op:'delete',path:[k]}:set([k],next[k])));};
...
function drawPropInspector(){
 ...
 $('remove-prop').onclick=()=>{const next=removeSceneEntity(store.document,'prop',p.id);transact(['props','contacts','game'].filter(k=>next[k]!==undefined).map(k=>set([k],next[k])));};
}
...
$('stage').onpointerdown=e=>{
 ...
 const propHit=e.target.closest('[data-prop]');if(!dragMode&&propHit){selectProp(propHit.dataset.prop);if(prop().attachment){toast('Edit attachment offsets in the Scene prop panel.');return;}const p=scenePoint(e.clientX,e.clientY);drag={type:'prop',start:p,origin:{x:prop().x,y:prop().y},next:null};$('stage').setPointerCapture(e.pointerId);return;}
...
$('stage').onkeydown=e=>{if(!e.key.startsWith('Arrow'))return;e.preventDefault();if(prop()&&!dragMode&&!e.shiftKey){if(prop().attachment){toast('Edit attachment offsets in the Scene prop panel.');return;}editProp({x:clamp(prop().x+(e.key==='ArrowLeft'?-4:e.key==='ArrowRight'?4:0),0,store.document.bounds.width),y:clamp(prop().y+(e.key==='ArrowUp'?-4:e.key==='ArrowDown'?4:0),0,store.document.bounds.height)});return;}if(e.shiftKey||dragMode){offset.x=clamp(offset.x+(e.key==='ArrowLeft'?-16:e.key==='ArrowRight'?16:0),-170,170);offset.y=clamp(offset.y+(e.key==='ArrowUp'?-16:e.key==='ArrowDown'?16:0),-100,100);$('stage').style.transform=`translate(${offset.x}px,${offset.y}px)`;}else if(actor())pose(baseValue()+(e.key==='ArrowLeft'||e.key==='ArrowDown'?-2:2));};
...
function tick(now){const dt=last===null||document.hidden?0:Math.min((now-last)/1000,.05);last=now;
 ...
 renderer.update(frame());animationGuides.update();meshTools.update(frame(),$('art').querySelector('svg'),inspector==='surface'&&!sceneSelection&&!selectedProp);updateTime();if(playing&&$('rotation')&&document.activeElement!==$('rotation-number')){$('rotation').value=baseValue();$('rotation-number').value=Math.round(baseValue()*(['bend','opacity'].includes(channel())?100:10))/(['bend','opacity'].includes(channel())?100:10);}
 const s=selectedRuntime()?.spring;
 behaviorTools.update(controller.frame());actorBehaviorTools.update?.(controller.frame());
