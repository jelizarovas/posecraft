...
const key = 'posecraft.studio.v2'+(demo?'.demo.'+demo.id:'');
...
function drawLighting(){
 const l=lightingConfig(store.document);
 $('inspector').innerHTML=`<div class="inspector-heading">Scene lighting</div><label class="face-auto"><input id="lighting-enabled" type="checkbox" ${l.enabled?'checked':''}>Enable lighting</label><label class="field">Character shading<select id="lighting-shading"><option value="gradient" ${l.shading==='gradient'?'selected':''}>Soft gradient</option><option value="cel" ${l.shading==='cel'?'selected':''}>Sharp / cel</option></select></label><div class="two-col">${field('Key color','lighting-color',l.color,'type="color"')}${field('Shadow color','lighting-shadowColor',l.shadowColor,'type="color"')}</div><div class="light-grid">${[['angle','Direction',-180,180,1],['elevation','Height',10,85,1],['intensity','Strength',0,2,.05],['ambient','Ambient',0,1,.05],['gloss','Highlights',0,1,.05],['softness','Softness',0,16,1],['floorShadow','Floor shadow',0,1,.05],['wallShadow','Wall shadow',0,1,.05],['reflection','Reflection',0,.8,.05]].map(([key,label,min,max,step])=>`<label class="field">${label} <output>${l[key]}</output><input data-light-field="${key}" type="range" min="${min}" max="${max}" step="${step}" value="${l[key]}"></label>`).join('')}</div><div class="two-col">${field('Floor line','lighting-floorY',l.floorY,'type="number" min="0" max="4096"')}${field('Wall edge','lighting-wallY',l.wallY,'type="number" min="0" max="4096"')}</div><p class="note">Stylized surface shading and planar shadows. Set the floor line at the feet; the wall edge clips the wall shadow. Zero softness removes blur. Reflections cost extra drawing.</p>`;
 ...
 $('lighting-shading').onchange=e=>change('shading',e.target.value);
 $('lighting-enabled').onchange=e=>change('enabled',e.target.checked);
 for(const key of ['color','shadowColor','floorY','wallY'])$('lighting-'+key).onchange=e=>{change(key,key.endsWith('Y')?+e.target.value:e.target.value);drawLighting();};
