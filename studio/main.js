...
function tick(now){const dt=last===null||document.hidden?0:Math.min((now-last)/1000,.05);last=now;
 ...
 if($('scene-time')&&document.activeElement!==$('scene-time'))$('scene-time').value=Math.min(60,controller.frame().time);
