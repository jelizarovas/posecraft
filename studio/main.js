...
function rebuild(){
 bottleControls?.dispose();pointers?.dispose();controller?.dispose();if(sceneSelection&&!['behavior','actor-behavior','objects','scroll','motion-layers'].includes(sceneSelection.kind)&&!store.document[{group:'groups',actor:'actors',prop:'props',emitter:'emitters'}[sceneSelection.kind]]?.some(e=>e.id===sceneSelection.id))sceneSelection=null;if(!prop())selectedProp=null;if(!actor())selected=store.document.actors[0]?.id;
