...
function cleanup(){token++;worker?.terminate();worker=null;controller?.dispose();controller=null;renderer?.dispose();renderer=null;renderedScene=null;ready=false;inFlight=false;pending=null;offset={x:0,y:0};drag=null;$('demo-stage').style.transform='';}
function select(id){
...
function show(f){
 ...
 $('demo-caption').textContent=episode?documentData.shots[f.shotIndex].name: selected.id==='drop-lab'?'Loose · Protect head · Brace':selected.id==='zero-gravity'?'Drag to move the container':'One cast, four different performances';
...
function drawControls(){
 ...
 if(selected.id==='drop-lab'){
  action('drop-cast','Drop',()=>{for(const a of targets()){controller.setBehavior(a.id,{mode:a.id==='loose'?'ragdoll':'protective',strategy:a.id==='brace'?'brace':'protect',resistance:.75,gravity:1});controller.interact(a.id,'drop');}});
  action('toss-cast','Toss',()=>{for(const a of targets()){controller.setBehavior(a.id,{mode:a.id==='loose'?'ragdoll':'protective',strategy:a.id==='brace'?'brace':'protect',resistance:.75,gravity:1});controller.interact(a.id,'toss');}});
...
$('demo-play').onclick=()=>{playing=!playing;last=null;if(controller)playing?controller.play():controller.pause();transport();};$('demo-reset').onclick=()=>select(selected.id);$('demo-scrub').oninput=e=>{time=+e.target.value;playing=false;requestFrame();transport();};
...
$('demo-stage').onpointerdown=e=>{if(selected.id!=='zero-gravity'||e.button!==0)return;resume();drag={x:e.clientX-offset.x,y:e.clientY-offset.y};$('demo-stage').setPointerCapture(e.pointerId);};
...
window.addEventListener('hashchange',()=>{if(location.hash.slice(1)!==selected.id)select(location.hash.slice(1));});document.addEventListener('visibilitychange',()=>{last=null;controller?.rebaseline();if(document.hidden){sound.mute();$('demo-sound').classList.remove('active');$('demo-sound').innerHTML=icon('play_arrow')+'Sound off';$('demo-sound').setAttribute('aria-label','Enable interaction sounds');}});window.addEventListener('pagehide',()=>{cleanup();sound.dispose();});
media.addEventListener('change',e=>{if(e.matches){playing=false;controller?.pause();transport();}last=null;});
select(location.hash.slice(1));
function tick(now){const dt=last===null?0:Math.min(.1,(now-last)/1000);last=now;if(playing&&!document.hidden){if(documentData.kind==='episode'){time=(time+dt)%episodeDuration(documentData);requestFrame();}else{controller?.sampleHost({...offset,time:now/1000});controller?.step(dt);}}requestAnimationFrame(tick);}requestAnimationFrame(tick);
