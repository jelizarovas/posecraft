// Reference media stays in this browser. Only captured frames enter IndexedDB.
let database;
...
export async function saveReference(id,blob){const database=await db();return new Promise((resolve,reject)=>{const tx=database.transaction('frames','readwrite');tx.objectStore('frames').put(blob,id);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);});}
