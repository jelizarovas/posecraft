...
`SceneController` and `WorkerSceneController` expose `triggerEnsemble("conversation" | "doze" | "meteor" | "share" | "share-missed" | "share-help" | "burn")`. Invalid events and scenes without an ensemble reject the command. Requests enter the existing first-minute replay log. Read `frame.ensemble.events` for the latest 32 event records and `frame.ensemble.sharing` for the handoff state. During sharing, `frame.ensemble.share` reports `phase`, `giver`, `receiver`, `giverHand`, `receiverHand`, optional `observer`, `noticed`, `owner` (null after consumption or release) and `contact` (the projected hand distance, null before reaching). Moving campers expose `groundY` for their current shadow and lighting support plane. Event records contain `type`, actor IDs, scene time and a detail string. Evaluated campers add `activity` and `heat`. No wall-clock timer or external service drives the ensemble.
