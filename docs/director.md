...
1. Build characters and props in Character Studio or [Draw / Rig](draw.md). In Director, select the regular Studio or Draw / Rig draft and choose **Use Studio scene**, or import an exported scene JSON. Each scene is a reusable set. **Copy set** creates an independent scene; multiple shots otherwise refer to the same set.
...
7. **Save** downloads a [portable project](project-files.md) with its reference images. **More → Export episode JSON** saves the episode without image bytes. The top download icon exports the currently rendered composition as SVG. The CLI can render any episode time without opening the browser.

Camera changes remain a draft until keyed. Source images and captured frames are alignment references; they do not become runtime character geometry automatically. Media is processed locally. Captured frames are stored in IndexedDB, while the episode stores a reference ID, filename, and source timestamp. Portable project files include the referenced images for another browser or device. Plain episode JSON still requires relinking those images. Original source videos are not bundled.
