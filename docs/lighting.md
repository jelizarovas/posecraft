...
For spatial actors with `groundY`, point lighting also uses that ground placement as approximate scene depth. The visible side's rotated normal controls direct light, so a fire behind a camper does not highlight the visible back. Ambient shading remains. This is a stylized depth calculation, not ray-traced occlusion. Actors without a ground placement keep the existing screen-plane lighting.
