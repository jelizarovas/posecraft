...
Tap to walk, double-tap to run, drag to pan, and pinch or scroll to zoom. Hold a destination or Alt-click to turn in place. The settings dialog can enter or exit browser fullscreen, and closes when you tap outside it or press Escape. Fullscreen includes the menu so its exit control stays accessible on mobile. Renderer (Canvas2D or WebGL2), movement mode (always walk, always run, or double-tap to run), waypoint recentering, resolution, shadows, motion effects, route arrows and FPS are saved in local storage. Changing the renderer reloads the scene with a one-time session checkpoint that preserves actor positions, object state and camera. An explicit renderer query parameter overrides the saved choice for that visit. The menu reports Canvas2D fallback if WebGL2 cannot run. A saved fullscreen preference resumes on the first game tap after reload because browsers require a user gesture.
