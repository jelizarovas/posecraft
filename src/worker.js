...
export class WorkerSceneController {
 ...
 setBehavior(actor,patch){const a=this.document.actors.find(a=>a.id===actor),b=behaviorConfig(patch);if(!a||!behaviorModes.includes(b.mode)||!['auto','brace','protect','curl'].includes(b.strategy)||typeof b.autoFace!=='boolean'||!Number.isFinite(b.resistance)||b.resistance<0||b.resistance>1||!Number.isFinite(b.gravity)||b.gravity<0||b.gravity>2||!Number.isFinite(b.bounce)||b.bounce<0||b.bounce>1)throw new Error('Invalid behavior settings.');this.command('setBehavior',[actor,patch]);}
