...
export function renderSVG(document, frame, { label = document.name, bones = false, limits = false, selectedActor, selectedJoint } = {}) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${document.bounds.width} ${document.bounds.height}" role="img" aria-label="${escape(label)}" width="100%" height="100%">${document.actors.map(actor => {
    ...
    return `<g data-actor="${actor.id}" transform="${placement(actor.transform)}">${pack.parts.map(part => {
