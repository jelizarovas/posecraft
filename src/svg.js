...
export function renderSVG(document, frame, { label = document.name, bones = false, limits = false, selectedActor, selectedJoint,physicsDebug=false,colliders=false,selectedProp,camera=frame.camera } = {}) {
  ...
  const renderActor=actor=>{
    ...
    return `<g data-actor="${actor.id}" visibility="${nodeVisible(document,actor)?'visible':'hidden'}" display="${nodeVisible(document,actor)?'inline':'none'}" data-activity="${escape(evaluated.activity||'')}" data-response="${escape(evaluated.response||'calm')}" data-emotion="${escape(evaluated.inputs?.emotion||'neutral')}" transform="${placement(evaluated.placement||actor.transform)}"><defs>${definitions}</defs><g data-artwork="" id="${prefix}-${actor.id}-artwork">${ordered.map(part => {
      ...
      return `<g data-slot="${part.id}" ${part.spatial?.mask?`clip-path="url(#${prefix}-${actor.id}-${part.spatial.mask})"`:''}><g data-joint="${part.joint}" data-selected="${actor.id===selectedActor && part.joint===selectedJoint}" transform="${fluid?.part(part,view?.transform||transform(evaluated.world[part.joint]))??(view?.transform||transform(evaluated.world[part.joint]))}"><path opacity="${(part.opacityChannel?evaluated.pose[part.opacityChannel]??1:1)*(view?.opacity??1)}" data-part="${part.id}" d="${escape(fluid?.paths[part.id]??view?.d??paint.d)}" fill="${light.enabled&&!actor.unlit&&surfaceRamp(paint.fill,light)?`url(#${prefix}-${actor.id}-${part.id}-surface)`:escape(paint.fill)}" stroke="${escape(part.stroke || 'none')}" stroke-width="${part.strokeWidth || 0}" stroke-linejoin="round" stroke-linecap="round" transform="${escape(fluid&&Object.hasOwn(fluid.paths,part.id)?'':paint.transform)}" visibility="${paint.visible&&view?.visible!==false&&!fluid?.hidden.has(part.id)?'visible':'hidden'}"/></g></g>`;
    }).join('')}</g>${bones ? pack.joints.map(j => `<g data-bone="${j.id}" transform="${fluid?.joint(j.id,spatial?spatialBone(spatial.world[j.id]):transform(evaluated.world[j.id]))??(spatial?spatialBone(spatial.world[j.id]):transform(evaluated.world[j.id]))}">${limits && j.id===selectedJoint && actor.id===selectedActor ? `<path data-limit="${j.id}" d="${limitArc(j,evaluated.pose[j.id+'.rotation'])}" fill="#9c71ff22" stroke="#7955be" stroke-width=".6" pointer-events="none"/>`:''}<path d="M0 0H${j.length || 12}" stroke="#7751bd" stroke-width="1.1" pointer-events="none"/><circle r="3" fill="${j.id===selectedJoint&&actor.id===selectedActor?'#7253bc':'#fff'}" stroke="#7253bc" stroke-width="1"/><circle data-handle="${j.id}" cx="${j.length || 12}" r="2.2" fill="#7253bc" stroke="#fff" stroke-width=".6"/></g>`).join('') : ''}</g>`;
  ...
  return {
    update(next) {
      ...
      for (const b of bindings) {
        ...
        if(spatial||fluid||b.hadFluid||inputKey!==b.inputKey){b.inputKey=inputKey;for (const {part,node} of b.paths) if (spatial||fluid||b.hadFluid||part.variants || part.showWhen) { const paint=appearance(part,b.actor,evaluated),view=spatial?.parts.get(part.id);attribute(node,'d',fluid?.paths[part.id]??view?.d??paint.d);attribute(node,'transform',fluid&&Object.hasOwn(fluid.paths,part.id)?'':paint.transform);attribute(node,'visibility',paint.visible&&view?.visible!==false&&!fluid?.hidden.has(part.id)?'visible':'hidden'); }}
        b.hadFluid=!!fluid;
