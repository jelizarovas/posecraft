...
export function renderSVG(document, frame, { label = document.name, bones = false, limits = false, selectedActor, selectedJoint,physicsDebug=false,colliders=false,selectedProp,camera=frame.camera } = {}) {
  ...
  const prefix='pc-view-'+(++spatialInstance),light=sampleLighting(lightingConfig(document),frame.time);
  const effects=light.enabled?`<rect data-light-wash="" width="${document.bounds.width}" height="${document.bounds.height}" fill="url(#${prefix}-wash)" pointer-events="none"/>`+document.actors.filter(a=>!a.unlit).map(actor=>{
   const evaluated=frame.actors.find(a=>a.id===actor.id),p=placement(evaluated.placement||actor.transform),href=`#${prefix}-${actor.id}-artwork`,contact=contactShadow(actor,document.packs[actor.pack],evaluated,light),anchor=actorAnchor(actor,evaluated),wall=wallProjection(light,anchor);
   return `<g data-light-effects="${actor.id}" pointer-events="none" aria-hidden="true">${light.wallShadow&&light.receiver==='corner'?`<g clip-path="url(#${prefix}-wall)" opacity="${light.wallShadow}"><g data-wall-shadow="" visibility="${wall?'visible':'hidden'}" transform="${wall||''}" filter="url(#${prefix}-shadow)"><use data-light-placement="" href="${href}" transform="${p}"/></g></g>`:''}${light.floorShadow?`<g clip-path="url(#${prefix}-floor)" opacity="${light.floorShadow}"><g filter="url(#${prefix}-shadow)"><g data-floor-shadow="" transform="${shadowProjection(light,anchor)}"><use data-light-placement="" href="${href}" transform="${p}"/></g></g></g><ellipse data-contact="${actor.id}" cx="${contact.cx}" cy="${light.floorY}" rx="${contact.rx}" ry="5" fill="${light.shadowColor}" opacity="${contact.opacity}" filter="url(#${prefix}-shadow)"/>`:''}${light.reflection?`<g data-reflection="" mask="url(#${prefix}-mirror-mask)" opacity="${light.reflection}"><g transform="translate(0 ${2*light.floorY}) scale(1 -1)"><use data-light-placement="" href="${href}" transform="${p}"/></g></g>`:''}</g>`;
  }).join(''):'';
  ...
  const renderActor=actor=>{
    ...
    return `<g data-actor="${actor.id}" data-response="${escape(evaluated.response||'calm')}" data-emotion="${escape(evaluated.inputs?.emotion||'neutral')}" transform="${placement(evaluated.placement||actor.transform)}"><defs>${definitions}</defs><g data-artwork="" id="${prefix}-${actor.id}-artwork">${ordered.map(part => {
      ...
      return `<g data-slot="${part.id}" ${part.spatial?.mask?`clip-path="url(#${prefix}-${actor.id}-${part.spatial.mask})"`:''}><g data-joint="${part.joint}" data-selected="${actor.id===selectedActor && part.joint===selectedJoint}" transform="${view?.transform||transform(evaluated.world[part.joint])}"><path opacity="${part.opacityChannel?evaluated.pose[part.opacityChannel]??1:1}" data-part="${part.id}" d="${escape(view?.d||paint.d)}" fill="${light.enabled&&!actor.unlit&&surfaceRamp(paint.fill,light)?`url(#${prefix}-${actor.id}-${part.id}-surface)`:escape(paint.fill)}" stroke="${escape(part.stroke || 'none')}" stroke-width="${part.strokeWidth || 0}" stroke-linejoin="round" stroke-linecap="round" transform="${escape(paint.transform)}" visibility="${paint.visible&&view?.visible!==false?'visible':'hidden'}"/></g></g>`;
  ...
  return `<svg data-scene-time="${frame.time}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${camera?.width||document.bounds.width} ${camera?.height||document.bounds.height}" role="img" aria-label="${escape(label)}" width="100%" height="100%">${camera?`<g data-camera="" transform="${cameraTransform(camera,document.bounds)}">`:''}${lightingDefinitions(light,document.bounds,prefix)}${document.actors.filter(a=>a.layer==='background').map(renderActor).join('')}${props}${effects}${document.actors.filter(a=>a.layer!=='background'&&a.layer!=='foreground').map(renderActor).join('')}${document.actors.filter(a=>a.layer==='foreground').map(renderActor).join('')}${light.enabled&&light.type==='point'&&light.showSource?`<g data-light-source="" transform="translate(${light.pointX} ${light.pointY})" pointer-events="none"><circle r="10" fill="${light.color}" stroke="#fff"/><circle r="18" fill="none" stroke="${light.color}" stroke-dasharray="2 4"/></g>`:''}${physicsDebug?`<g data-physics-debug="" pointer-events="none">${physicsOverlay(frame)}</g>`:''}${camera?'</g>':''}</svg>`;
}
export function mountSVG(element, document, frame, options) {
  element.innerHTML = renderSVG(document, frame, options);
  let previous;const baseLight=lightingConfig(document);
  ...
  return {
    update(next) {
      if(next===previous)return;previous=next;attribute(element.firstElementChild,'data-scene-time',next.time);const light=sampleLighting(baseLight,next.time);
      ...
      for (const b of bindings) {
        ...
        for(const {part,node} of b.paths)if(part.opacityChannel)attribute(node,'opacity',evaluated.pose[part.opacityChannel]??1);
