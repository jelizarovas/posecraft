...
export const ensembleEvents=['conversation','doze','meteor','share','share-missed','share-help','burn','fire-off','fire-relight','fire-on','food-throw','face-shoo','food-ready'];
...
export class CampfireEnsemble {
 ...
 actorSignal(id,source){const a=this.members.find(v=>v.id===id);if(!a)return undefined;const busy=a.retired||a.stage!==null||this.inFire(a)||this.share&&(a.id===this.share.giver||a.id===this.share.receiver&&this.share.noticed)||a.reaction?.type==='food-throw';if(source==='heat')return a.heat;if(source==='busy')return !!busy;if(source==='heating')return this.fire.lit&&!busy;if(source==='distracted')return a.mode!=='fire'||this.meteors.some(m=>this.time-m.at>.45&&this.time-m.at<4);}
 setActorHeat(id,value){const a=this.members.find(v=>v.id===id);if(a)a.heat=clamp(value,0,2);}
 drainActorEvents(){const events=this.actorEvents;this.actorEvents=[];return events;}
 cookingEvent(a,event){if(this.authoredCooking.has(a.id)&&this.actorEvents.length<64)this.actorEvents.push({actor:a.id,event});}
 ready(a){a.burned=false;a.stage=13.5;this.event('ready',[a.id],a.mode);a.mode='fire';this.cookingEvent(a,'cooking-busy');}
 burn(a,reason){if(!this.fire.lit)return;a.heat=1.04;a.burned=true;a.stage=12;a.startleAt=this.time;a.mode='fire';this.event('burn',[a.id],reason);this.event('burn-startle',[a.id]);this.cookingEvent(a,'cooking-burn');}
 ...
 finishShare(){const s=this.share;for(const a of this.members)if([s.giver,s.receiver].includes(a.id)){a.stage=20;a.mode='fire';a.heat=0;a.burned=false;this.cookingEvent(a,'cooking-reset');}this.share=null;}
 tickShare(){
 ...
 tick(){
  ...
  for(const a of this.members){if(a.retired){a.reaction=null;a.thrown=null;continue;}if(a.reaction&&this.time>=a.reaction.until){a.reaction=null;a.thrown=null;}if(a.reaction?.type==='food-throw'){if(a.thrown&&this.time-a.thrown.at>a.thrown.duration)a.thrown=null;continue;}if(this.inFire(a))continue;if(this.share&&(a.id===this.share.giver||a.id===this.share.receiver&&this.share.noticed))continue;
   if(a.stage!==null){const before=a.stage;a.stage+=.05;if(before<16.5&&a.stage>=16.5&&this.time>=this.nextShare&&!this.share){this.offer(true);this.nextShare=this.time+90+this.random()*90;}if(a.stage>=24){a.stage=null;a.heat=0;a.burned=false;a.mode='fire';a.distraction='';this.cookingEvent(a,'cooking-reset');}continue;}
   if(this.time>=a.until)a.mode='fire';const distracted=a.mode!=='fire'||this.meteors.some(m=>this.time-m.at>.45&&this.time-m.at<4);
   if(this.authoredCooking.has(a.id))continue;if(!this.fire.lit)continue;if(distracted&&a.heat>.6)a.distraction=a.mode==='fire'?'watching meteor':a.mode;a.heat+=.05*(.048+a.index*.002);
