...
self.onmessage=({data:m})=>{
 try{
  ...
  for(const [method,...args] of m.commands){
   try{
    if(!['fluidInput','pointer','dispatch','setVariable','triggerEnsemble','walkTo','setInput','setBehavior','interact','previewClip','clearPreview','setAcceleration','rebaseline','play','pause','reset','seek'].includes(method))throw new Error('Unknown simulation command.');
