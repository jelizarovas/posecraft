...
export type BehaviorPayload={object?:string;game?:string;actor?:string;x?:number;y?:number};
export type BehaviorAction={type:'object';command:import('./scene-objects.js').ObjectCommand}|{type:'perform';activity:string}|{type:'add';variable:string;value:number}|{type:'set';variable:string;value:boolean|number}|{type:'input';actor:string;input:string;value:boolean|number|string}|{type:'emitter';emitter:string;enabled:boolean}|{type:'ensemble'|'event';event:string;actor?:string};
