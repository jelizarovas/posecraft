...
export interface DrawingMesh {actor:string;part:string;joint:string;matrix:Matrix;vertices:Array<{x:number;y:number;z:number;depth:number}>;triangles:number[][];fill:Material;opacity:number;visible:boolean;masked:boolean;outlined:boolean}
export interface EvaluatedDrawing {version:1;time:number;width:number;height:number;units:Array<{id:string;depth:number|null;layer:string;commands:DrawingPath[]}>;meshes:DrawingMesh[];stats:{actors:number;paths:number;vertices:number;triangles:number};capabilities:CapabilityReport}
