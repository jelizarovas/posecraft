...
export function createRestAction3D({rig,roles,frame,duration=4}){
 ...
 return {duration,beats:[{id:'rest',label:'Catch breath',start:0,end:duration}],sample(time){time=Math.max(0,Math.min(duration,time));const pose=structuredClone(source.pose),envelope=Math.sin(Math.PI*time/duration)**2,angle=Math.sin(time*2*Math.PI*.6)*.025*envelope;pose[chest]={...pose[chest],rotation:new Quaternion(...base).multiply(new Quaternion().setFromAxisAngle(new Vector3(1,0,0),angle)).toArray()};let world=evaluateRig3D(compiled,pose,source.placement);
   const head=rig.joints.find(j=>j.id===roles.head);if(head){const parent=head.parent?world[head.parent].rotation:source.placement.rotation,look=new Quaternion().setFromAxisAngle(new Vector3(0,1,0),Math.sin(time/duration*Math.PI*1.6)*.42*envelope).multiply(new Quaternion(...world[head.id].rotation));look.premultiply(new Quaternion().setFromAxisAngle(new Vector3(1,0,0),-.12*envelope));pose[head.id]={...pose[head.id],rotation:new Quaternion(...parent).invert().multiply(look).normalize().toArray()};world=evaluateRig3D(compiled,pose,source.placement);}
   const diagnostics=[];for(const key of ['leftLeg','rightLeg']){const name=roles[key]??key,chain=compiled.chains[name];if(!chain)continue;const target={position:source.world[chain.tip].position,rotation:source.world[chain.tip].rotation},result=solveTwoBone3D(compiled,pose,name,target,{placement:source.placement});Object.assign(pose,result.pose);world=result.world;diagnostics.push({id:key,chain:name,active:true,target,actual:world[chain.tip],...result.diagnostics});}return {...source,time,duration,phase:time===duration?'complete':'rest',pose,world,diagnostics,contacts:diagnostics,valid:diagnostics.every(d=>d.status==='solved')};}};
}
