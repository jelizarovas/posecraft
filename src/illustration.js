...
export class IllustrationController {
  constructor(document,{reducedMotion=false,ensembleFactory,contactSolver,behaviorFactory,pointerFactory}={}){
    ...
    for(const [needed,provider,name] of [[document.ensemble,ensembleFactory,'ensemble'],[document.contacts?.length,contactSolver,'contacts'],[document.behaviorGraph&&document.presentation!=='sequence',behaviorFactory,'behaviors'],[document.interactions?.length,pointerFactory,'pointer interactions']])if(needed&&!provider)throw new Error(`Missing illustration provider: ${name}.`);
    this.providers={ensembleFactory,contactSolver,behaviorFactory,pointerFactory};this.reducedMotion=reducedMotion;this.playing=true;this.animationPlaying=true;this.listeners=new Set();this.reset();
  }
  reset(){
  ...
  tick(){
    this.time+=STEP;if(this.behaviors&&this.ensemble){this.ensemble.advance(this.time,this.excluded());for(const {event,...payload} of this.ensemble.drainEvents?.()||[])this.behaviors.dispatch(event,payload);}this.behaviors?.tick(STEP);this.pointers?.step(STEP);
...
export function mountIllustration(element,document,{host=element,reducedMotion='system',onEvent,onError,label,autoplay=true,mountPointers,...providers}={}){
