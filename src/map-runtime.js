import {MapRoutePreparation,routeHeading,angleDelta} from './map-motion.js';
...
export class MapController {
 ...
 schedulePaths(){
  ...
  this.timer=setTimeout(()=>{
   ...
   for(const job of pending){if(allocated>=4)break;if(job.planning&&!job.pathJob){try{job.pathJob=new MapPathJob(this.map,this.index,job.start,job.goals);allocated++;}catch(error){job.planning=false;this.finish(job,error);}}}
 ...
 accept(job,result){
  ...
  job.preparation=new MapRoutePreparation(this.index,actor,result.path);this.schedulePaths();
 }
  ...
  for(const job of [...this.active.values()]){
   ...
   for(let n=0;n<steps&&job.index<job.route.length;n++){
    ...
    while(job.index<job.route.length&&travel>0){
     ...
     if(d>1e-9){actor.x+=dx/d*length;actor.y+=dy/d*length;actor.phase=(actor.phase+length*Math.PI*2)%(Math.PI*2);actor.travelFacing=Math.atan2(dy,dx);}
