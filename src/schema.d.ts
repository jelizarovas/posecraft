...
export interface CharacterPack { name:string; joints:Joint[]; parts:Part[]; clips:Record<string,Clip>; inputs:Record<string,Input>; initial:string; states:Record<string,{clip:string;transitions?:{to:string;duration:number;when:{input:string;equals:string|number|boolean}}[]}>; reaction?:{joint:string;strength:number;stiffness:number;damping:number}; physics?:PhysicsProfile; expressions?:Record<string,Record<string,number>>; appearanceDefaults?:Record<string,string>; description?:string; provenance?:Record<string,string> }
