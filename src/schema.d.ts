...
export interface SceneProp {depth?:{value:number};group?:string;hidden?:boolean;layer?:'background'|'characters'|'foreground';id:string;name:string;x:number;y:number;width:number;height:number;rotation:number;fill:string;collider:{enabled:boolean;width:number;height:number;x:number;y:number;friction:number;bounce:number}}
