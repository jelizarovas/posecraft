...
export interface Part { id:string; joint:string; d:string; fill:string; channel?:string; transform?:string; stroke?:string; strokeWidth?:number }
export interface CharacterPack { name:string; joints:Joint[]; parts:Part[]; clips:Record<string,Clip>; inputs:Record<string,Input>; initial:string; states:Record<string,{clip:string;transitions?:{to:string;duration:number;when:{input:string;equals:string|number|boolean}}[]}>; reaction?:{joint:string;strength:number;stiffness:number;damping:number}; provenance?:Record<string,string> }
export interface Actor {id:string;name:string;pack:string;transform:{x:number;y:number;scale:number;rotation:number};appearance?:Record<string,string>}
