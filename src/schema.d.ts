...
export interface SpatialPart {depth?:number;order?:number;center?:[number,number];thickness?:number;axis?:'x'|'y';facing?:'front'|'back';mask?:string;surface?:{x:number;width:number;depth:number};morph?:{channel:string;target:string}}
export interface Part { spatial?:SpatialPart; id:string; joint:string; d:string; fill:string; channel?:string; transform?:string; stroke?:string; strokeWidth?:number; variantInput?:string; variants?:Record<string,{d?:string;transform?:string;visible?:boolean}>;showWhen?:{input:string;equals:string|number|boolean} }
