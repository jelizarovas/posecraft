...
export interface PosecraftHandle {play():void;pause():void;reset():void;seek(time:number):void;interact(actor:string,type:Interaction,strength?:number):void;readonly controller:SceneController|WorkerSceneController|undefined}
export interface PosecraftProps {scene:SceneDocument;execution?:'worker'|'main';inputs?:Record<string,Record<string,string|number|boolean>>;behavior?:Record<string,BehaviorSettings>;onEvent?:(event:SceneEvent)=>void;onError?:(error:Error)=>void;motion?:(time:number)=>{x:number;y:number;teleport?:boolean};hostRef?:RefObject<HTMLElement|null>;reducedMotion?:boolean|'system';label?:string;className?:string;style?:CSSProperties}
