...
export function createBenchScoot3D({from,to,armReach,legReach,seatY,sole,stance,wideStance,entryFootZ,exitFootZ,direction,tempo}){
 const count=Math.max(1,Math.ceil(Math.abs(to-from)/Math.min(.17,armReach*.34))),step=(to-from)/count;
 const prepare=1.4/tempo,cycleDuration=2.15/tempo,finish=1.5/tempo,duration=prepare+count*cycleDuration+finish;
 const footOffset=Math.min(.23,legReach*.27),liftHeight=Math.min(.035,armReach*.07),lean=55*Math.PI/180;
 const handZ=i=>from+step*(i+.5)+.28;
 function sample(time){
  const t=Math.max(0,Math.min(duration,time));
  let handLift=0,cycle=0,stage='prepare',u=0,hipZ=from,seatLift=0,pitch=0,handBlend=0,handPositionZ=handZ(0),feet=[0,1].map(()=>({z:entryFootZ,y:sole,x:direction==='back'?stance:wideStance,active:true}));
  if(t<prepare){
   u=t/prepare;pitch=lean*smooth(u/.7);handBlend=smooth((u-.55)/.45);
   for(let i=0;i<2;i++){const stepU=clamp((u-i*.35)/.65),f=smooth(stepU);feet[i]={x:mix(direction==='back'?stance:wideStance,wideStance,f),z:mix(entryFootZ,from+footOffset,f),y:sole+Math.sin(Math.PI*stepU)*.045,active:stepU===0||stepU===1};}
   ...
   cycle=count-1;stage='finish';u=(t-prepare-count*cycleDuration)/finish;hipZ=to;pitch=lean*(1-smooth((u-.35)/.65));handBlend=1-smooth(u/.5);handPositionZ=handZ(cycle);
   for(let i=0;i<2;i++){const stepU=clamp((u-i*.3)/.7),f=smooth(stepU);feet[i]={x:mix(wideStance,direction==='forward'?stance:wideStance,f),z:mix(from+step*(count-1)+footOffset,exitFootZ,f),y:sole+Math.sin(Math.PI*stepU)*.035,active:stepU===0||stepU===1};}
  }
