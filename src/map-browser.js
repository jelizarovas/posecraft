...
const groundShadowStamps=new WeakMap();
function drawPropGroundShadow(ctx,map,p){
  const doc=ctx.canvas.ownerDocument;let stamps=groundShadowStamps.get(doc);
  if(!stamps){stamps=new Map();groundShadowStamps.set(doc,stamps);}
  let stamp=stamps.get(p.kind);
  if(!stamp){
    stamp=doc.createElement('canvas');stamp.width=192;stamp.height=64;const c=stamp.getContext('2d');c.scale(2,2);c.translate(40,10);
    const tree=p.kind==='tree',house=p.kind==='house',rx=house?34:tree?24:13;
    ellipse(c,tree?13:7,5,rx+4,tree?10:7,'#19271c12');
    ellipse(c,tree?11:5,4,rx,tree?8:5,'#19271c20');
    ellipse(c,0,1,house?25:tree?11:10,house?7:3.5,'#14251c58');stamps.set(p.kind,stamp);
  }
  const q=projectMap(map,propCenter(p)),s=map.tileSize.width/64;
  ctx.drawImage(stamp,q.x-40*s,q.y-10*s,96*s,32*s);
}
...
export function mountMap(element,map,{onEvent,onError,execution='worker',autoplay=true,reducedMotion='system',followOnMove=false,onCameraChange}={}){
  ...
  function render(now=0){const paintStart=performance.now(),rect={x:camera.x-width/(2*zoom),y:camera.y-height/(2*zoom),width:width/zoom,height:height/zoom},dpr=pixelRatio(),key=[camera.x,camera.y,width,height,zoom,dpr,artRevision].join(':');
    ...
    if(propsKey!==sceneryKey){
      ...
      for(const entry of drawList)drawPropGroundShadow(sceneryContext,map,entry.item);
      for(const entry of drawList)drawProp(sceneryContext,map,entry.item,frame.objects?.[entry.item.id],{art,shadow:false});
  ...
  function wheel(e){e.preventDefault();const p=following?{x:width/2,y:height/2}:local(e),before=toWorld(p.x,p.y);zoom=clamp(zoom*Math.exp(-e.deltaY*.001),.45,5);camera={x:before.x-(p.x-width/2)/zoom,y:before.y-(p.y-height/2)/zoom};invalidate();}
