...
export function addSpatialRig(pack,id,{studies=true}={}){
 if(!['ona','dummy'].includes(id))throw new Error('Depth rig presets support Ona and Dummy.');
 if(pack.spatial)return pack;
 pack.spatial=true;
 ...
 if(id==='ona'){
  ...
  Object.assign(front,{d:'M0 0',transform:'',strokeWidth:1.5,variants:{none:{visible:false}},spatial:{order:96,hairShell:{width:46,height:34,depth:32,y:-12}}});
  const rear=pack.parts.find(p=>p.id==='hair-rear-cap');rear.variants=Object.fromEntries(pack.inputs.hair.options.map(h=>[h,{visible:false}]));
  const tail=pack.parts.find(p=>p.id==='hair-back');tail.spatial={depth:-23,center:[45,-10],order:-10};tail.variants.bob={visible:false};
 }
 if(!studies)return pack;
