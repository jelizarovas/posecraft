...
export interface ActionVariant {weightInfluences?:{variable:string;weight:number}[];when?:{variable:string;op:'eq';value:number};onSuccess?:ActivityEffect[];id:string;clip:string;start?:number;end?:number;weight:number;speed:{min:number;max:number};offsets?:Record<string,{min:number;max:number}>}
...
export const ACTIVITY_LIMITS:Readonly<{activities:number;variants:number;offsets:number;modifiers:number;weightInfluences:number}>;
