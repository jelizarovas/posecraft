...
export class GamePerformance{
 ...
 available(entry){return nodeVisible(this.controller.document,entry.actor)&&!entry.physics&&entry.behavior.mode==='animated'&&!entry.preview&&(!entry.recovery||['home','blocked'].includes(entry.recovery.phase));}
 relinquish(actor){this.held.delete(actor);this.ground.delete(actor);const s=this.active.get(actor);if(s)this.finish(s,'Actor is now controlled by another motion or preview.',true);}
 finish(s,error,cancelled=false){if(this.active.get(s.actor)!==s)return;this.active.delete(s.actor);this.controller.emit({type:error?'actor.command.failed':'actor.command.completed',actor:s.actor,request:s.request,command:s.type,...(error?{error,cancelled}:{})});}
 cancel(actor,request,reason='Command cancelled.'){const s=this.active.get(actor);if(!s||request&&s.request!==request)return false;const frame=this.controller.frame().actors.find(a=>a.id===actor);if(s.root&&frame)this.ground.set(actor,{[s.root+'.x']:frame.pose[s.root+'.x']??0,[s.root+'.y']:frame.pose[s.root+'.y']??0});this.held.delete(actor);this.finish(s,reason,true);return true;}
 ...
 command(input){
  ...
  const visible=this.controller.frame().actors.find(a=>a.id===c.actor);this.cancel(c.actor,null,'Command replaced.');
  ...
  const s={...c,time:0,entry,root,base:{...frame.pose},duration:c.type==='action'?entry.pack.clips[c.clip].duration:c.type==='look'?c.duration:0};this.active.set(c.actor,s);this.held.delete(c.actor);
  ...
  if(root)this.ground.set(c.actor,{[root+'.x']:frame.pose[root+'.x']??0,[root+'.y']:frame.pose[root+'.y']??0});
  if(c.type==='move'){
 ...
 tick(dt,immediate=false){
  ...
  for(const s of [...this.active.values()]){
   if(!this.available(s.entry)){this.relinquish(s.actor);continue;}
   s.time=immediate?s.duration:Math.min(s.duration,s.time+dt);
   if(s.motion){s.motion.tick(immediate?s.duration:dt);this.ground.set(s.actor,{[s.root+'.x']:s.motion.pose[s.root+'.x'],[s.root+'.y']:s.motion.pose[s.root+'.y']});}
   if(s.time+1e-9>=s.duration){this.held.set(s.actor,s);this.finish(s);}
  }
 }
 apply(frame){
  if(!this.active.size&&!this.held.size&&!this.ground.size)return frame;
  return {...frame,actors:frame.actors.map(actor=>{
   const s=this.active.get(actor.id)??this.held.get(actor.id),anchor=this.ground.get(actor.id);if(!s&&!anchor)return actor;
   const entry=s?.entry??this.controller.actors.find(a=>a.actor.id===actor.id);if(!this.available(entry)){this.relinquish(actor.id);return actor;}
