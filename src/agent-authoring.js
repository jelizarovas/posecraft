...
import {retimeSceneClip} from './timeline-editing.js';
...
export function proposeSceneEdit(document,{expectedRevision,operations}={}){
 ...
 if(operations.some(op=>op?.type==='retime-clip')){
  if(operations.length!==1)throw Error('retime-clip must be the only operation in its proposal. Apply other edits separately against the resulting revision.');
  const op=operations[0],result=retimeSceneClip(document,{packId:op.pack,clipId:op.id,duration:op.duration});
  return {format:'posecraft-edit-proposal',version:1,scene:document.id,expectedRevision:result.expectedRevision,commands:copy(result.commands),summary:[{type:op.type,pack:op.pack,id:op.id,description:`Retime clip to ${op.duration}s, including its markers and supported timing references.`,notes:copy(result.notes)}],diagnostics:diagnoseScene(result.document)};
 }
 ...
 for(const op of operations){if(!record(op))throw Error('Expected a semantic operation.');
  if(op.type==='create-clip'){
   ...
   const value={duration:op.duration,loop:op.loop??false,tracks:copy(op.tracks),...(op.events!==undefined?{events:copy(op.events)}:{})};draft.packs[op.pack].clips[op.id]=value;commands.push({op:'set',path:['packs',op.pack,'clips',op.id],value});summary.push({type:op.type,pack:op.pack,id:op.id,description:`Create ${op.duration}s clip with ${Object.keys(op.tracks||{}).length} tracks.`});
