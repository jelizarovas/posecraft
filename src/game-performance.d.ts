...
export type GameCommand=({type:'action';clip:string}|{type:'look';target:{x:number;y:number};joint:string;maxAngle:number;duration:number}|{type:'move';x:number}|{type:'cancel'})&{actor:string;request:string};
