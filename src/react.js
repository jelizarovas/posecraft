...
export const Posecraft = forwardRef(function Posecraft({ scene, inputs = {}, behavior = {}, onEvent, onError, motion, hostRef, reducedMotion = 'system', label, className, style }, ref) {
  ...
  useEffect(()=>{try{for(const [actor,settings] of Object.entries(behavior))instance.current?.setBehavior(actor,settings);}catch(error){errors.current?.(error);}},[behavior,scene,reducedMotion,label,hostRef,!!motion]);
