...
export interface Frame {time:number;actors:{id:string;pose:Record<string,number>;world:Record<string,{x:number;y:number;rotation:number;endX:number;endY:number}>;state:string;spring:{x:number;y:number;vx:number;vy:number}}[]}
...
export class SceneController {
 ...
 document:SceneDocument;time:number;playing:boolean;reducedMotion:boolean;log:unknown[];size?:{width:number;height:number};
