...
export function shadowVectors(light,anchor={x:0,y:0}){const length=1/Math.tan(light.elevation*rad);return light.type==='point'?{x:(anchor.x-light.pointX)/light.pointHeight,y:(light.floorY-light.pointY-light.pointHeight)/light.pointHeight}:{x:-Math.cos(light.angle*rad)*length*.65,y:Math.sin(light.angle*rad)*length*.65};}
