...
export interface GameActorBinding {actions?:Record<string,string>;reactions?:Record<string,{action?:string;emotion?:string}>;gaze?:{joint:string;maxAngle?:number};speech?:boolean}
export interface GameBindings {anchors?:Record<string,GameTarget>;actors?:Record<string,GameActorBinding>}
export interface ResolvedGameActorBindings {actions:Record<string,string>;reactions:Record<string,{action?:string;emotion?:string}>;gaze?:{joint:string;maxAngle?:number};speech:boolean}
export interface GameSceneManifest {schemaVersion:1;id:string;actors:Array<{id:string;name:string;actions:string[];reactions:string[];canSpeak:boolean;canLook:boolean;locomotion:'ground-x'|'none';anchors:string[]}>;objects:Array<{id:string;properties:['enabled'];commands:string[]}>;anchors:string[];events:string[]}
