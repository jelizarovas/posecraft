...
export async function exportNativeBenchHTML(project,{runtimeURL='./runtime/native-three-player.js',assetURL}={}){
  ...
  const {assertWorkoutProject3D}=await import('./workout-project-3d.js');
  const data=structuredClone(project?.kind==='workout3d'?assertWorkoutProject3D(project):assertBenchProject3D(project));
...
export const exportNativeWorkoutHTML=exportNativeBenchHTML;
