...
export function validateBehaviorGraph(document,check){
 ...
 const validateActions=(actions,p)=>{check(Array.isArray(actions)&&actions.length<=16,p,'Expected at most 16 actions.');if(!Array.isArray(actions))return;
...
export class BehaviorRuntime {
 ...
 reset(){this.time=0;this.rng=this.graph.seed>>>0;this.variables={...this.graph.variables};this.queue=[];this.emitterOverrides={};this.transitions=0;this.droppedEvents=0;this.enter(this.graph.initial,{});return this.snapshot();}
 ...
 runActions(actions,payload){for(const original of actions){const action={...original};if(action.actor==='$actor')action.actor=payload.actor;
   if(action.type==='set')this.variables[action.variable]=action.value;
 ...
 setVariable(name,value){validateBehaviorVariable(this.graph,name,value);this.variables[name]=value;}
 tick(dt){if(!Number.isFinite(dt)||dt<0)throw Error('Behavior elapsed time must be finite and nonnegative.');this.time+=dt;let transitions=0,events=0;
 ...
 snapshot(){return {state:this.state,variables:{...this.variables},enteredAt:this.enteredAt,time:this.time,transitions:this.transitions,droppedEvents:this.droppedEvents,pendingEvents:this.queue.length};}
}
