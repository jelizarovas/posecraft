...
export interface BottleFluidFrame {actor:string;vessel:string;contents:string;waterPath:string;surfacePath:string;waterPolygon:FluidPoint[];area:number;bulkArea:number;splashArea:number;targetArea:number;surfaceSamples:FluidPoint[];waveHeights:number[];normal:FluidPoint;level:number;paths:Record<string,string>;hidden:string[];bottle:{x:number;y:number;rotation:number;vx:number;vy:number;angularVelocity:number};ship:{joint:string;x:number;y:number;rotation:number;scale:number};slosh:number;waveAmplitude:number;splashPath:string;foamPath:string;waveParticles:number;wind:number;contacts:number;grabbed:number}
