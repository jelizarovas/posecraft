...
try{
 ...
 await page.goto(base+'/demos.html#gym-routine');await page.locator('#scene-full-set').click();await page.locator('#demo-scrub').fill('42');await page.locator('#demo-scrub').dispatchEvent('input');await page.waitForFunction(()=>Math.abs(+document.querySelector('#demo-art svg')?.dataset.sceneTime-42)<.03);
 ...
 await page.screenshot({path:'test-results/gym-skin-release-studio.png'});assert.deepEqual(errors,[]);console.log('Atlas connected mesh verified in bench preview, downloaded scene and Studio vertex editing with undo.');
