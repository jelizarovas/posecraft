...
try{
 await page.goto(base);await page.locator('#art [data-part]').first().waitFor();await page.locator('#file').setInputFiles({name:'timeline.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(fixture))});await page.locator('#edit-keys').click();
 ...
 await page.locator('#keys-close').click();const durable=await keys();await page.reload();await page.locator('#art [data-part]').first().waitFor();assert.deepEqual(await keys(),durable);await page.locator('#edit-keys').click();
