...
test('the full back view hides facial artwork and exposes the authored back',()=>{
 const q=fixture();try{const {actor}=q.at('turnaround',6),p=spatialParts(q.doc.packs.atlas,actor);for(const id of ['eyes','nose','eyebrows','left-pec','right-pec'])assert.equal(p.parts.get(id).visible,false,id+' visible from back');for(const id of ['back-scapula-left','back-scapula-right','back-spine'])assert.equal(p.parts.get(id).visible,true,id+' missing');}finally{q.controller.dispose();}
});
