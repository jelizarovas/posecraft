...
test('gym knees gain depth while projected feet stay on their authored support',()=>{
 const q=setup();try{for(const [clip,time]of [['jump-grab-left',1.8],['jump-grab-right',1.8],['full-set',6.8],['full-set',31.2]]){const {world:w}=q.pose(clip,time);for(const side of ['left','right']){const hip=w[side+'Thigh'],knee=w[side+'Calf'],foot=w[side+'Foot'];assert.ok(knee.z-hip.z>5,clip+' knee remains flat');assert.ok(Math.abs(knee.x-(hip.x+foot.x)/2)<15,clip+' knee splays sideways');}}
  const {world:w}=q.pose('full-set',31.2),walk=gymWalk(31.2,29,36,180,710);for(const side of ['left','right'])assert.ok(Math.hypot(w[side+'Foot'].x-walk.feet[side].x,w[side+'Foot'].y-walk.feet[side].y)<1.5,'depth rotation moves support foot');
