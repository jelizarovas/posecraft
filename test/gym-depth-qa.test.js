...
test('free gym arms bend in depth outside the torso instead of folding inward',()=>{
 const q=setup();try{for(const [clip,time,side,sign]of [['jump-grab-left',1.8,'right',1],['jump-grab-right',1.8,'left',-1],['full-set',1,'left',-1],['full-set',1,'right',1]]){const {world:w,frame,actor}=q.pose(clip,time),roll=actor.pose['pelvis.rotation']*Math.PI/180,transverse=(w[side+'Lower'].x-w.torso.x)*Math.cos(roll)+(w[side+'Lower'].y-w.torso.y)*Math.sin(roll);assert.ok(transverse*sign>22,clip+' elbow folds through torso');assert.ok(w[side+'Lower'].z-w[side+'Upper'].z>2,clip+' relaxed elbow retains depth');for(const c of frame.contacts.filter(c=>c.active))assert.ok(c.error<.1);}}finally{q.controller.dispose();}
});
