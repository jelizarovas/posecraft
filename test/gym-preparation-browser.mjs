...
try{
 await fs.mkdir('test-results',{recursive:true});await page.route('**/favicon.ico',route=>route.fulfill({status:204,body:''}));await page.goto(base+'/demos.html?legacy=1#gym-routine');await page.getByLabel('Action variations',{exact:true}).selectOption(review.clip);await page.waitForFunction(label=>document.querySelector('#demo-caption').textContent.includes(label),review.label);assert.equal(await page.locator('#demo-scrub').getAttribute('max'),String(review.duration));assert.equal(await page.locator('#demo-beat').inputValue(),'grip');
 ...
 await page.setViewportSize({width:390,height:844});await page.goto(base+'/demos.html?legacy=1#gym-routine');await page.locator('#gym-grip-review').selectOption(review.clip);await seek(2.48);assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1&&document.documentElement.scrollHeight<=innerHeight+1));await page.screenshot({path:'test-results/gym-preparation-mobile.png'});assert.deepEqual(errors,[]);console.log('Preparation browser passed:three distinct breath-outs,mouth/air visibility,slowplay/loop/reset,Studio clip and compact mobile.');
