...
test('unavailable authored flames cannot heat food or signal restoration in full and lightweight runtimes',()=>{
 for(const patch of [{enabled:false},{rate:0},{opacity:0}]){const d=createCampfire();Object.assign(d.emitters.find(e=>e.id==='fire-flame'),patch);const full=new SceneController(d),lite=new IllustrationController(d,{ensembleFactory:CampfireEnsemble,behaviorFactory:BehaviorRuntime,pointerFactory:ScenePointerInteraction});for(const c of [full,lite]){const initial=c.frame(),cold=c.seek(25);assert.equal(cold.ensemble.fire.lit,false);assert.equal(cold.ensemble.fire.available,false);assert.equal(cold.ensemble.fire.blocked,true);assert.equal(cold.ensemble.fire.heat,0);assert.ok(!cold.ensemble.events.some(e=>e.type==='fire-lit'));assert.ok(cold.ensemble.events.some(e=>e.type==='fire-restart-blocked'));for(const id of d.ensemble.members)assert.equal(actor(cold,id).heat,actor(initial,id).heat);c.dispatch('ignite-fire');assert.equal(c.frame().ensemble.fire.lit,false);assert.ok(sampleEmitters(d,25,c.frame()).every(e=>e.particles.every(p=>p.opacity===0)));assert.equal(sampleLighting(lightingConfig(d,c.frame()),25).intensity,0);}assert.deepEqual(full.frame().ensemble,lite.frame().ensemble);assert.deepEqual(full.frame().emitterOverrides,lite.frame().emitterOverrides);full.dispose();lite.dispose();}
});

test('graph flame disable controls heat and visuals; an explicit enable restores both runtimes',()=>{
 const d=createCampfire();d.behaviorGraph.handlers.push({event:'disable-source',actions:[{type:'emitter',emitter:'fire-flame',enabled:false}]},{event:'enable-source',actions:[{type:'emitter',emitter:'fire-flame',enabled:true}]});const full=new SceneController(d),lite=new IllustrationController(d,{ensembleFactory:CampfireEnsemble,behaviorFactory:BehaviorRuntime,pointerFactory:ScenePointerInteraction});for(const c of [full,lite]){c.dispatch('disable-source');const cold=c.seek(25);assert.equal(cold.ensemble.fire.available,false);assert.equal(cold.ensemble.fire.blocked,true);assert.equal(cold.emitterOverrides['fire-flame'].enabled,false);assert.ok(!cold.ensemble.events.some(e=>e.type==='fire-lit'));c.dispatch('enable-source');c.step(.05);const warm=c.frame();assert.equal(warm.ensemble.fire.lit,true);assert.equal(warm.ensemble.fire.available,true);assert.equal(warm.ensemble.fire.blocked,false);assert.equal(warm.emitterOverrides['fire-flame'].enabled,true);assert.equal(warm.behavior.state,'warm');assert.ok(warm.ensemble.events.some(e=>e.type==='fire-lit'));}assert.deepEqual(full.frame().ensemble,lite.frame().ensemble);full.dispose();lite.dispose();
});
