...
try{
 await fs.mkdir('test-results',{recursive:true});await page.goto(base+'/demos.html?legacy=1#gym-routine');await page.locator('#gym-thirsty').waitFor();assert.equal(await page.locator('#demo-scrub').isVisible(),false);assert.match(await page.locator('#demo-time').textContent(),/Paused|Live illustration/);
