...
try{
 ...
 await page.setViewportSize({width:390,height:844});await page.goto(base+'/demos.html#gym-routine');await page.locator('#gym-grip-review').selectOption('pull-lead-left');assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1&&document.documentElement.scrollHeight<=innerHeight+1));assert.deepEqual(errors,[]);console.log('Asymmetric gym browser passed: eight previews, fatigue weights and grips in export, Studio clips and phone layout.');
