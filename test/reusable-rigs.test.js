...
test('existing gallery Ona and Dummy packs use reusable depth artwork and scene folders',()=>{
 for(const entry of demoCatalog.filter(e=>!['a-little-handoff','game-of-catch','campfire-night','gym-routine','ship-in-a-bottle','loveseat-stairs'].includes(e.id))){const doc=createDemo(entry.id);for(const scene of doc.kind==='episode'?Object.values(doc.scenes):[doc]){assert.ok(scene.groups.length>=2);assert.ok(scene.lighting.enabled);for(const [id,p] of Object.entries(scene.packs)){if(!['ona','dummy'].includes(id))continue;assert.ok(p.spatial,entry.id+' '+id);if(id==='ona'){assert.ok(p.parts.some(part=>part.spatial?.hairShell));assert.ok(p.joints.some(j=>j.id==='rightWrist'));assert.ok(p.parts.some(part=>part.spatial?.softLimb));}else assert.ok(p.clips.idle.tracks['leftFoot.yaw']);}}}
});
