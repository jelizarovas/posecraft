...
try{
 ...
 await page.evaluate(async()=>{
   ...
   window.fixture={format:'posecraft-map',version:1,id:'art',name:'Art',width:64,height:64,seed:1,tileSize:{width:72,height:36},terrain:Array(4096).fill(0),props:[{id:'chest',kind:'chest',x:5,y:5,width:1,height:1}],actors:[{id:'hero',x:5.5,y:4.5,speed:3,color:'#e65bb1'}],art:{images:{tree:{src:'fixture-sprite.png',width:128,height:128,anchorX:.5,anchorY:1},treeAlias:{src:'fixture-sprite.png',width:128,height:128,anchorX:.5,anchorY:1},grass:{src:'fixture-grass.png',width:64,height:64,anchorX:0,anchorY:0},unused:{src:'fixture-missing.png',width:64,height:64,anchorX:0,anchorY:0}},props:{chest:['tree','treeAlias']},terrain:{grass:'grass'}}};
 ...
 assert.equal(await page.evaluate(()=>clickEvents.at(-1).target),'chest','Opaque interactive sprite pixels select the prop');
 await page.evaluate(()=>a.controller.cancel('hero'));
 ...
 await page.evaluate(async()=>{window.failures=[];const failed=structuredClone(fixture);failed.art.images.tree.src='fixture-missing.png';failed.art.props.chest=['tree'];window.failed=mountMap(document.querySelector('#a'),failed,{execution:'main',onError:e=>failures.push(e.message)});await window.failed.ready;});
