...
try{
 ...
 const before=await page.evaluate(()=>mapDemo.view.stats());assert.deepEqual(before.art,{requested:10,loaded:10,failed:0});assert.ok(before.visibleProps<before.totalProps/10);assert.ok(before.visibleTiles<before.totalTiles/10);
 ...
 await page.waitForTimeout(100);await page.screenshot({path:'test-results/map-woodland-lake.png'});
