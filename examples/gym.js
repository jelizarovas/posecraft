...
export function gymPose(time,mode='workout'){
 ...
 // Keep the directional head ahead of the narrow neck, behind the pull-up rail at depth 10.
 pose['root.x']=x;pose['root.y']=y;pose['floor-depth.y']=(gymSceneDepths.bench-383)*sit;pose['pelvis.z']=30;pose['torso.x']=torsoX;pose['torso.y']=torsoY+breath;pose['torso.rotation']=benchRoll*lying-15*tired-7*anticipation;pose['torso.yaw']=(walking?.direction||1)*40*(walking?.blend||0)+60*lying;pose['pelvis.rotation']=benchRoll*lying;pose['pelvis.yaw']=(walking?.direction||1)*30*(walking?.blend||0)+60*lying;pose['head.z']=9.5;pose['head.rotation']=20*lying+8*tired;pose['head.yaw']=-12*lying+(walking?.direction||1)*10*(walking?.blend||0);pose['head.pitch']=-10*effort+12*tired+7*anticipation;
