...
function withCampfireCooking(scene){
 scene.actorBehaviors??=[];
 for(const [i,actor]of scene.ensemble.members.entries()){
  if(scene.actorBehaviors.some(s=>s.actor===actor))continue;
  const edge=(id,from,to,variable,op,value,delay=0)=>({id,from,to,after:{min:delay,max:delay},weight:1,...(variable?{when:{variable,op,value}}:{})});
  scene.actorBehaviors.push({id:actor+'-cooking',actor,sensors:[{variable:'heating',source:'campfire.heating'},{variable:'distracted',source:'campfire.distracted'},{variable:'busy',source:'campfire.busy'}],outputs:[{variable:'heat',source:'campfire.heat'}],rates:[{variable:'heat',perSecond:.048+i*.002,when:{variable:'heating',op:'eq',value:true}}],graph:{seed:20260917+i*7919,variables:{heat:[.45,.05,.3,.6][i],heating:true,distracted:false,busy:false},variableBounds:{heat:{min:0,max:2}},initial:'roasting',states:{roasting:{actions:[]},available:{actions:[]},blocked:{actions:[]},temperature:{actions:[]},attention:{actions:[]},waiting:{actions:[]},ready:{actions:[{type:'ensemble',event:'food-ready',actor:'$actor'}]},burned:{actions:[{type:'ensemble',event:'burn',actor:'$actor'}]},holding:{actions:[]}},handlers:[{event:'cooking-reset',actions:[{type:'set',variable:'heat',value:0}]},{event:'cooking-burn',actions:[{type:'set',variable:'heat',value:1.04}]}],edges:[edge('ready-temperature','roasting','available','heat','gte',.72,.05),edge('warm-enough','available','temperature','heating','eq',true),edge('wait-for-heat','available','blocked','heating','eq',false),edge('check-again','blocked','available',null,null,null,.05),edge('burn-threshold','temperature','burned','heat','gte',1.03),edge('attention-check','temperature','attention','heat','lt',1.03),edge('notice-food','attention','ready','distracted','eq',false),edge('miss-food','attention','waiting','distracted','eq',true),edge('check-food','waiting','available',null,null,null,.05),edge('serve-hold','ready','holding',null,null,null,.05),edge('burn-hold','burned','holding',null,null,null,.05),{id:'start-serving',from:'*',to:'holding',event:'cooking-busy',weight:1},{id:'forced-burn',from:'*',to:'holding',event:'cooking-burn',weight:1},{id:'fresh-food',from:'*',to:'roasting',event:'cooking-reset',weight:1}]}});
 }
 scene.requiredFeatures=[...new Set([...(scene.requiredFeatures||[]),'actor-behaviors'])];return scene;
}
/** Explicit migration preserves existing actor graphs and character artwork. */
export function convertCampfireCooking(document){if(document.ensemble?.type!=='campfire')throw Error('Expected a campfire scene.');const scene=withCampfireCooking(structuredClone(document));scene.revision++;return scene;}
