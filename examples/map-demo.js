...
export function mountMapDemo(documentData, {playing=true,onPlayingChange=()=>{},onError=()=>{}}={}) {
  ...
  $('demo-controls').innerHTML='<div class="demo-actions map-actions"><button id="map-home"><span class="material-symbols-outlined" aria-hidden="true">restart_alt</span>Center hero</button><button id="map-chest">Find chest</button><button id="map-inn">Visit inn</button><button id="map-seed"><span class="material-symbols-outlined" aria-hidden="true">add</span>New map</button><button id="map-save">Save</button><button id="map-restore">Restore</button><label>Zoom <input id="map-zoom" type="range" min="0.45" max="5" step="0.05" value="1" aria-label="Map zoom"></label></div>';
