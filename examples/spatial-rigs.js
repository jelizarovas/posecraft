...
export function addSpatialRig(pack,id){
 ...
 for(const [index,p] of pack.parts.entries()){
  ...
  if(id==='ona'){
   ...
   if(p.id==='hair-back')Object.assign(p.spatial,{depth:-18,thickness:.8,axis:'x'});
   if(p.id==='hair-front')Object.assign(p.spatial,{depth:32,facing:'front',thickness:.8,axis:'x'});
 ...
 if(id==='ona'){
  ...
  pack.parts.push({id:'hair-rear-cap',joint:'head',d:'M-39 -21C-41 -48 -25 -55 0 -55C25 -55 41 -48 39 -21C25 -12 -25 -12 -39 -21Z',fill:'#65504a',channel:'hair',variantInput:'hair',variants:Object.fromEntries(hair.map(h=>[h,h==='none'?{visible:false}:{}])),spatial:{depth:-5,facing:'back',thickness:.8,axis:'x'}});
 }else{
